...
for "_i" from 1 to _quantity do {
	...
	if (count _lightExplosives > 0) then {
		_explosives pushBack [selectRandom _lightExplosives, 2, 1];
	};	

	if (random 1 > 0.5 && count _heavyExplosives > 0) then {
		_explosives pushBack [selectRandom _heavyExplosives, 1, 1];
	};

	if (random 1 > 0.5 && count _atMines > 0) then {
		_explosives pushBack [selectRandom _atMines, 1, 1];
	};

	if (random 1 > 0.5 && count _apMines > 0) then {
		_explosives pushBack [selectRandom _apMines, 1, 1];
	};
