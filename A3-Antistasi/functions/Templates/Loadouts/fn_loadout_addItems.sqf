...

{
	...
	private _batchLoadTotal = 0;
	{
		...
		_item pushBack ([_item select 0] call A3A_fnc_loadout_itemLoad);
		//Add total load to the item array
		_item pushBack ((_item select 1) * (_item select 2));
		//Final array format: [class, count, single item load, all items load]
		_items pushBack _item;
		//Add total load to the overall load of the batch
		_batchLoadTotal = _batchLoadTotal + (_item select 3);
 	} forEach _rawItems;
