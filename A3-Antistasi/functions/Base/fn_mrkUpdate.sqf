...
{
    //Activate that once the new garrison system is merged (if ever)
    //private _garrisonCount = [([_marker] call A3A_fnc_getOver), true] call A3A_fnc_countGarrison;
	//_text = format [": %1", _garrisonCount];
    _text = if (count (garrison getVariable [_marker,[]]) > 0) then {format [": %1", count (garrison getVariable [_marker,[]])]} else {""};
