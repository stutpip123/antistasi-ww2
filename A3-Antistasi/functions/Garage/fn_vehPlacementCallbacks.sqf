	...
	case "GARAGE": {
		switch (_callbackType) do {
			...
			case CALLBACK_VEH_PLACED_SUCCESSFULLY: {
                ...
                {
                    private _marker = [_garageVeh] call A3A_fnc_isStaticWeaponOnMarker;
                    ...
                    [_garageVeh, _marker] call A3A_fnc_addStaticToGarrison;
                };
			};
	...
	case "BUYFIA": {
		switch (_callbackType) do {
			...
			case CALLBACK_VEH_PLACED_SUCCESSFULLY: {
				...
				if (_purchasedVeh isKindOf "StaticWeapon") then
                {
                    private _marker = [_purchasedVeh] call A3A_fnc_isStaticWeaponOnMarker;
                    ["Static", format ["You deployed the static weapon at %1. If there are militia units stationated here, they will man this weapon shortly!", [_marker] call A3A_fnc_localizar]] call A3A_fnc_customHint;
                    [_purchasedVeh, _marker] call A3A_fnc_addStaticToGarrison;
                };
