...
/*The next section is for defining the weapons that can be mounted on a given vehicle.
  ...
  This makes sure that the vehicle can mount all appropriate weapons from all mods loaded.

  This is done by adding array entries made up of the vehicle model path, followed by the correct array.
  The entries are usually separated by an inline comment that says what vehicle the next entry is for, as model paths aren't always obvious.
*/
...
//That covers everything, you should make your file by replacing values in an already complete file rather than using this as the active files will have the proper commenting there already. Using this one would leave a tonne of unnecessary comments in the file.
